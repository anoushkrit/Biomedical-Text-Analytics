...
import json
Entrez.email = 'anoushkritgoel@gmail.com'  # CONTACT EMAIL NECESSARY FOR ENTREZ ACCESS
...
def topicmodel(trainingtexts, testtexts, numtopics=5, numwords=3):
    ...
    for i in range(int(numtopics)):
        topics_termids.append(
            lda.get_topic_terms(i, topn=10))  # list tuples (termid, probability), top n terms per topic
    ...
    for i in range(len(testtexts)):
        ...
        docs_topics.append(
            listtopntopics)  # Ranked topics per doc, tuples with probability, probability > minimum 0.01 by default
        try:
...
def pubyearhistogram(pubdate, topicplotted='cardiomyocyte'):
    ...
    try:
        ...
        print(
            "Publication dates for {} records ranging within {} years, {} to {}".format(totallength, maxtime - mintime,
                                                                                        mintime, maxtime))
        binwidth = 1
        sidespace = 5
        ...
        plt.close()
        try:
            print('No histogram for {}, all records published in {}'.format(topicplotted,
                                                                            max(topicdocdf.ReleaseDate.dt.year)))
        except:
...
def topicdistributionhistogram(topicarray, wholearray):
    with plt.style.context('bmh'):
        ...
        print("Topic disribution and frequency per year, for {} records ranging within {} years, {} to {}".format(
            totallength, (maxtime - mintime), mintime, maxtime))
        binwidth = 1
        sidespace = 5
        plt.hot()
        plt.hist(topicarray, histtype='barstacked', bins=np.arange(mintime, maxtime + binwidth, binwidth), alpha=0.5,
                 label=((range(int(numsearchtopics)))))
        plt.xlim((mintime - sidespace), (maxtime + sidespace))
